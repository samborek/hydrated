...
import { AggregationTimeRange } from "@/squid";
export const omnipoolVolumeQuery = (squidSdk) => queryOptions({
...
export const stablepoolVolumeQuery = (squidSdk) => queryOptions({
...
export const xykVolumeQuery = (squidSdk, addresses) => queryOptions({
    queryKey: ["xykVolumes"],
    queryFn: async () => {
        const data = await squidSdk.XykVolume({
            filter: {
                poolIds: addresses.map((address) => safeConvertSS58toPublicKey(address)),
                period: AggregationTimeRange["24H"],
            },
        });
