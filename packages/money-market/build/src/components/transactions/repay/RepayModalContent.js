...
import { API_ETH_MOCK_ADDRESS, InterestRate, synthetixProxyByChainId, } from "@aave/contract-helpers";
import { calculateHealthFactorFromBalancesBigUnits, USD_DECIMALS, } from "@aave/math-utils";
import { Alert, Separator, Stack, SummaryRow, } from "@galacticcouncil/ui/components";
...
export const RepayModalContent = ({ poolReserve, userReserve, symbol: modalSymbol, tokenBalance, nativeBalance, debtType, }) => {
    ...
    const [assets, setAssets] = useState([tokenToRepayWith]);
    ...
    const debt = debtType === InterestRate.Stable
    ...
    if (repayWithATokens) {
        ...
        const normalizedWalletBalance = Big(tokenToRepayWith.balance).minus(userReserve?.reserve.symbol.toUpperCase() ===
    ...
    const isMaxExceeded = !!amount && BigNumber(amount).gt(maxAmountToRepay.toString());
    const handleChange = (value) => {
        ...
        if (maxSelected && (repayWithATokens || maxAmountToRepay.eq(debt))) {
            ...
            setRepayMax(safeAmountToRepayAll.lt(balance)
    ...
    useEffect(() => {
        ...
        if (currentMarketData.v3 &&
            ...
            const maxBalance = BigNumber.max(aTokenBalance, BigNumber.min(aTokenBalance, debt).toString());
            repayTokens.push({
                address: poolReserve.underlyingAsset,
    ...
    const amountAfterRepay = Big(debt)
        .minus(amount || "0")
        .toString();
    const amountAfterRepayInUsd = bigShift(Big(amountAfterRepay)
    ...
    if (amount) {
        let collateralBalanceMarketReferenceCurrency = user?.totalCollateralUSD || "0";
        if (repayWithATokens && usageAsCollateralEnabledOnUser) {
            collateralBalanceMarketReferenceCurrency = Big(user?.totalCollateralUSD || "0").minus(Big(reserve.priceInUSD).mul(amount));
        }
        const remainingBorrowBalance = Big(user?.totalBorrowsUSD || "0").minus(Big(reserve.priceInUSD).mul(amount));
        const borrowBalanceMarketReferenceCurrency = Big.max(remainingBorrowBalance, 0);
        const calculatedHealthFactor = calculateHealthFactorFromBalancesBigUnits({
            collateralBalanceMarketReferenceCurrency: collateralBalanceMarketReferenceCurrency.toString(),
            borrowBalanceMarketReferenceCurrency: borrowBalanceMarketReferenceCurrency.toString(),
    ...
    return (_jsxs(_Fragment, { children: [_jsx(AssetInput, { name: "repay-amount", value: amount, onChange: handleChange, usdValue: usdValue.toString(), symbol: tokenToRepayWith.symbol, assets: assets, onSelect: setTokenToRepayWith, isMaxSelected: isMaxSelected, maxValue: maxAmountToRepay.toString(), error: isMaxExceeded ? "Remaining debt exceeded" : undefined }), _jsx(Separator, { mx: "var(--modal-content-inset)" }), _jsxs(Stack, { separated: true, separator: _jsx(Separator, { mx: "var(--modal-content-inset)" }), withTrailingSeparator: true, children: [_jsx(SummaryRow, { label: "Remaining debt", content: _jsx(ValueDetail, { align: "flex-end", value: formatCurrency(amountAfterRepay, {
                                symbol: poolReserve.symbol,
                            }), subValue: formatCurrency(amountAfterRepayInUsd.toString()) }) }), shouldRenderHealthFactor && (_jsx(SummaryRow, { label: "Health Factor", content: _jsx(HealthFactorChange, { healthFactor: healthFactor, futureHealthFactor: futureHealthFactor }) })), maxRepayWithDustRemaining && (_jsx(Alert, { sx: { my: 14 }, variant: "warning", description: "You don't have enough funds in your wallet to repay the full amount. If you proceed to repay with your current amount of funds, you will still have a small borrowing position in your dashboard." }))] }), _jsx(RepayActions, { maxApproveNeeded: safeAmountToRepayAll.toString(), poolReserve: poolReserve, amountToRepay: isMaxSelected ? repayMax : amount, poolAddress: repayWithATokens
                    ? poolReserve.underlyingAsset
                    : (tokenToRepayWith.address ?? ""), symbol: modalSymbol, debtType: debtType, repayWithATokens: repayWithATokens, blocked: isMaxExceeded })] }));
};
