...
export const SupplyActions = memo(({ amountToSupply, poolAddress, symbol, blocked, decimals, className, }) => {
    const { formatCurrency } = useAppFormatters();
    const queryClient = useQueryClient();
    const [supply, estimateGasLimit, currentMarketData] = useRootStore((state) => [
        state.supply,
        state.estimateGasLimit,
        state.currentMarketData,
    ]);
    const { approvalTxState, mainTxState, loadingTxns, setLoadingTxns, setApprovalTxState, setMainTxState, setGasLimit, setTxError, close, } = useModalContext();
    const { refetchPoolData, refetchIncentiveData, refetchGhoData } = useBackgroundDataProvider();
    const { sendTx } = useWeb3Context();
    const { data: approvedAmount, refetch: fetchApprovedAmount, isRefetching: fetchingApprovedAmount, isFetchedAfterMount, } = usePoolApprovedAmount(currentMarketData, poolAddress);
    useEffect(() => {
    ...
    return (_jsx(TxActionsWrapper, { blocked: blocked, mainTxState: mainTxState, approvalTxState: approvalTxState, requiresAmount: true, amount: amountToSupply, preparingTransactions: loadingTxns || !approvedAmount, actionText: _jsxs("span", { children: ["Supply ", symbol] }), actionInProgressText: _jsxs("span", { children: ["Supplying ", symbol] }), handleAction: action, requiresApproval: requiresApproval, className: className }));
});
