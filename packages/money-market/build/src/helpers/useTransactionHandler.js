...
export const useTransactionHandler = ({ handleGetTxns, handleGetPermitTxns, tryPermit = false, permitAction, skip, protocolAction, deps = [], toasts, }) => {
    const queryClient = useQueryClient();
    const { approvalTxState, mainTxState, setMainTxState, gasLimit, setGasLimit, loadingTxns, setLoadingTxns, setTxError, close, } = useModalContext();
    const { sendTx } = useWeb3Context();
    const { refetchPoolData, refetchIncentiveData, refetchGhoData } = useBackgroundDataProvider();
    const [walletApprovalMethodPreference, jsonRpcProvider] = useRootStore((state) => [state.walletApprovalMethodPreference, state.jsonRpcProvider]);
    ...
    const processTx = async ({ tx, errorCallback, }) => {
        try {
            await tx();
            try {
    ...
    const action = async () => {
        if ((!usePermit || !approvalTxes) && actionTx) {
            try {
                ...
                return processTx({
                    tx: () => sendTx(params, toasts, protocolAction),
