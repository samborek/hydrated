...
import { formatGhoReserveData, formatGhoUserData, formatUserSummaryWithDiscount, USD_DECIMALS, } from "@aave/math-utils";
...
import { reserveSortFn, selectCurrentBaseCurrencyData, selectCurrentReserves, selectCurrentUserEmodeCategoryId, selectCurrentUserReserves, selectEmodes, selectFormattedReserves, selectUserSummaryAndIncentives, } from "@/store/poolSelectors";
import { useRootStore } from "@/store/root";
import { getUserApyValues } from "@/utils";
import { formatGhoReserve, GHO_SUPPORTED_MARKETS, GHO_SYMBOL, isGho, } from "@/utils/ghoUtilities";
/**
 ...
 */
export const AppDataProvider = ({ children, externalApyData }) => {
    const currentTimestamp = useCurrentTimestamp(60);
    const { currentAccount } = useWeb3Context();
    const { currentMarket } = useProtocolDataContext();
    const { formatReserve } = useAppFormatters();
    const [reserves, baseCurrencyData, userReserves, userEmodeCategoryId, eModes, ghoReserveData, ghoUserData, ghoReserveDataFetched, formattedReserves, userSummary,] = useRootStore(useShallow((state) => [
    ...
    const formattedPoolReserves = formattedReserves.map((reserve) => isGho(reserve)
        ? formatGhoReserve(reserve, formattedGhoReserveData)
        : formatReserve(reserve));
    let ghoBorrowCap = "0";
    let aaveFacilitatorRemainingCapacity = Math.max(formattedGhoReserveData.aaveFacilitatorRemainingCapacity - 0.000001, 0);
    ...
    if (GHO_SUPPORTED_MARKETS.includes(currentMarket)) {
        ...
        const marketReferenceCurrencyPriceInUsd = Big(baseCurrencyData?.marketReferenceCurrencyPriceInUsd || "0");
        if (formattedGhoUserData.userDiscountedGhoInterest > 0 &&
            ...
            const userSummaryWithDiscount = formatUserSummaryWithDiscount({
                ...
                user,
                marketReferenceCurrencyPriceUSD: bigShift(baseCurrencyData?.marketReferenceCurrencyPriceInUsd || "0", -USD_DECIMALS).toNumber(),
            });
            user = {
                ...user,
    ...
    const isUserHasDeposits = user.userReservesData.some((userReserve) => userReserve.scaledATokenBalance !== "0");
    const userApyValues = getUserApyValues(user, formattedPoolReserves, formattedGhoUserData, formattedGhoReserveData);
    return (_jsx(AppDataContext.Provider, { value: {
