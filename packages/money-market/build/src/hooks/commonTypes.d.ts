...
import { ComputedUserReserve, formatReservesAndIncentives, FormatUserSummaryAndIncentivesResponse } from "@aave/math-utils";
export type HookOpts<T, V> = {
    select?: (originalValue: T) => V;
    refetchInterval?: number | false | (() => number | false);
    staleTime?: number;
};
export type ComputedReserveData = ReturnType<typeof formatReservesAndIncentives>[0] & ReserveDataHumanized & {
    iconSymbol: string;
