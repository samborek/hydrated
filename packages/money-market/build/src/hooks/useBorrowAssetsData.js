...
import { assetCanBeBorrowedByUser, getMaxAmountAvailableToBorrow, getMaxGhoMintAmount, } from "@/utils/getMaxAmountAvailableToBorrow";
export const useBorrowAssetsData = () => {
    const { currentMarket } = useProtocolDataContext();
    const { user, reserves, marketReferencePriceInUsd, ghoReserveData, loading } = useAppDataContext();
    ...
    const sortedReserves = useMemo(() => {
        const tokensToBorrow = reserves
            ...
            const availableBorrows = account
                ...
                : 0;
            const availableBorrowsInUSD = bigShift(Big(availableBorrows)
            ...
            return {
                ...
                stableBorrowRate: reserve.stableBorrowRateEnabled && reserve.borrowingEnabled
        ...
        const maxBorrowAmount = Big(user?.totalBorrowsMarketReferenceCurrency || "0").plus(user?.availableBorrowsMarketReferenceCurrency || "0");
        const collateralUsagePercent = maxBorrowAmount.eq(0)
            ? "0"
            : Big(user?.totalBorrowsMarketReferenceCurrency || "0")
                .div(maxBorrowAmount)
                .toString();
        const borrowReserves = !account ||
...
const getAvailableBorrows = (user, reserve, ghoReserveData) => {
    return ghoReserveData
        ? Math.min(Number(getMaxGhoMintAmount(user, reserve)), ghoReserveData.aaveFacilitatorRemainingCapacity)
