 ...
 */
export function useCurrentTimestamp(updateInterval = 15) {
    const [timeTravel, setTimeTravel] = useState(0);
    const [timestamp, setTimestamp] = useState(0);
    useEffect(() => {
        const intervalHandlerID = setInterval(() => setTimestamp(Math.floor(Date.now() / 1000) + timeTravel), 1000 * updateInterval);
