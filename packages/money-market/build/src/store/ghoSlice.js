...
export const createGhoSlice = (set, get) => {
    return {
        ghoReserveData: {
        ...
        displayGho: ({ symbol, currentMarket, }) => {
        ...
        ghoUserQualifiesForDiscount: (futureBorrowAmount = "0") => {
            ...
            if (!ghoReserveDataFetched || !ghoUserDataFetched)
                return false;
            ...
            const minBorrowBalanceForDiscount = Number(normalize(ghoReserveData.ghoMinDebtTokenBalanceForDiscount, 18));
            const stkAaveBalance = Number(normalize(ghoUserData.userDiscountTokenBalance, 18));
            const minStkAaveBalanceForDiscount = Number(normalize(ghoReserveData.ghoMinDiscountTokenBalanceForDiscount, 18));
            return (borrowBalance + Number(futureBorrowAmount) >=
        ...
        ghoMarketConfig: () => {
            ...
            const { GHO_TOKEN_ADDRESS: ghoTokenAddress, GHO_UI_DATA_PROVIDER: uiGhoDataProviderAddress, } = get().currentMarketData.addresses;
