 ...
 * @param user
 */
export function getMaxAmountAvailableToBorrow(poolReserve, user, rateMode) {
    const availableInPoolUSD = poolReserve.availableLiquidityUSD;
    const availableForUserUSD = Big.min(user.availableBorrowsUSD, availableInPoolUSD);
    const availableBorrowCap = poolReserve.borrowCap === "0"
        ? Big(ethers.constants.MaxUint256.toString())
        : Big(Number(poolReserve.borrowCap)).minus(poolReserve.totalDebt);
    const availableLiquidity = Big.max(Big.min(poolReserve.formattedAvailableLiquidity, availableBorrowCap.toString()), 0);
    const availableForUserMarketReferenceCurrency = Big(user?.availableBorrowsMarketReferenceCurrency || 0).div(poolReserve.formattedPriceInMarketReferenceCurrency);
    let maxUserAmountToBorrow = Big.min(availableForUserMarketReferenceCurrency.toString(), availableLiquidity);
    if (rateMode === InterestRate.Stable) {
        maxUserAmountToBorrow = Big.min(maxUserAmountToBorrow, 
    ...
    maxUserAmountToBorrow.gte(availableForUserMarketReferenceCurrency) ||
        ...
        (user.isInIsolationMode &&
            ...
            bigShift(Big(user.isolatedReserve?.debtCeiling || "0").minus(user.isolatedReserve?.isolationModeTotalDebt || "0"), -(user.isolatedReserve?.debtCeilingDecimals || 0))
                .mul("0.99")
 ...
 * @param user
 */
export function getMaxGhoMintAmount(user, poolReserve) {
    const userAvailableBorrows = Big(user?.availableBorrowsMarketReferenceCurrency || 0);
    const availableBorrowCap = poolReserve.borrowCap === "0"
        ? Big(ethers.constants.MaxUint256.toString())
        : Big(Number(poolReserve.borrowCap)).minus(poolReserve.totalDebt);
    const maxAmountUserCanMint = Big.max(Big.min(userAvailableBorrows, availableBorrowCap), 0);
    const shouldAddMargin = 
    /**
     * When a user has borrows we assume the debt is increasing faster then the supply.
     * That's a simplification that might not be true, but doesn't matter in most cases.
...
export function assetCanBeBorrowedByUser({ borrowingEnabled, isActive, borrowableInIsolation, eModeCategoryId, isFrozen, isPaused, }, user) {
