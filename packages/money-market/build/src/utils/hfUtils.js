...
export function calculateHFAfterSwap({ fromAmount, fromAssetData, fromAssetUserData, toAmountAfterSlippage, toAssetData, user, }) {
    const reserveLiquidationThreshold = user.isInEmode && user.userEmodeCategoryId === fromAssetData.eModeCategoryId
    ...
    if (fromAssetUserData.usageAsCollateralEnabledOnUser &&
    ...
    if ((!user.isInIsolationMode && !toAssetData.isIsolated) ||
        ...
        hfEffectOfToAmount = calculateHealthFactorFromBalancesBigUnits({
            collateralBalanceMarketReferenceCurrency: Big(toAmountAfterSlippage)
                ...
                .toString(),
            borrowBalanceMarketReferenceCurrency: user.totalBorrowsMarketReferenceCurrency,
            currentLiquidationThreshold: user.isInEmode &&
    ...
    return {
        ...
        hfAfterSwap: user.healthFactor === "-1"
            ? Big("-1")
...
export const calculateHFAfterRepay = ({ user, amountToReceiveAfterSwap, amountToSwap, fromAssetData, toAssetData, repayWithUserReserve, debt, }) => {
    // it takes into account if in emode as threshold is different
    const reserveLiquidationThreshold = user.isInEmode && user.userEmodeCategoryId === fromAssetData.eModeCategoryId
        ? fromAssetData.formattedEModeLiquidationThreshold
        : fromAssetData.formattedReserveLiquidationThreshold;
    ...
    if (repayWithUserReserve?.usageAsCollateralEnabledOnUser &&
    ...
    const fromAmountInMarketReferenceCurrency = Big(Big.min(amountToReceiveAfterSwap, debt))
        .mul(toAssetData.priceInUSD)
        .toString();
    let debtLeftInMarketReference = Big(user.totalBorrowsUSD).minus(fromAmountInMarketReferenceCurrency);
    ...
    const hfRealEffectOfFromAmount = fromAssetData.reserveLiquidationThreshold !== "0" &&
...
export const calculateHFAfterWithdraw = ({ user, userReserve, poolReserve, withdrawAmount, }) => {
    let totalCollateralInETHAfterWithdraw = Big(user.totalCollateralMarketReferenceCurrency);
    ...
    const reserveLiquidationThreshold = user.isInEmode && user.userEmodeCategoryId === poolReserve.eModeCategoryId
        ? poolReserve.formattedEModeLiquidationThreshold
        : poolReserve.formattedReserveLiquidationThreshold;
    if (userReserve?.usageAsCollateralEnabledOnUser &&
        poolReserve.reserveLiquidationThreshold !== "0") {
        const amountToWithdrawInEth = Big(withdrawAmount || "0").mul(poolReserve.formattedPriceInMarketReferenceCurrency);
        totalCollateralInETHAfterWithdraw = totalCollateralInETHAfterWithdraw.minus(amountToWithdrawInEth);
        liquidationThresholdAfterWithdraw = Big(user.totalCollateralMarketReferenceCurrency)
        ...
        const calculatedHFAfterWithdraw = calculateHealthFactorFromBalancesBigUnits({
            collateralBalanceMarketReferenceCurrency: totalCollateralInETHAfterWithdraw.toString(),
            borrowBalanceMarketReferenceCurrency: user.totalBorrowsMarketReferenceCurrency,
...
export const calculateHFAfterSupply = ({ user, poolReserve, supplyAmount, }) => {
    const supplyAmountInEth = Big(supplyAmount).mul(poolReserve.formattedPriceInMarketReferenceCurrency);
    ...
    const liquidationThresholdAfter = user
        ? Big(user.totalCollateralMarketReferenceCurrency)
            .mul(user.currentLiquidationThreshold)
            .plus(supplyAmountInEth.mul(poolReserve.formattedReserveLiquidationThreshold))
    ...
    if (user &&
        ...
        const calculatedHFAfterDeposit = calculateHealthFactorFromBalancesBigUnits({
            collateralBalanceMarketReferenceCurrency: totalCollateralMarketReferenceCurrencyAfter.toString(),
