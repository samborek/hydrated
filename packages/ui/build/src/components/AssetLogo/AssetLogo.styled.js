...
const calculateCircleOffset = (percentage, diameter) => {
...
const decorations = (thickness, padding) => createVariants((theme) => ({
    none: css ``,
    atoken: css `
...
export const SAssetBadge = styled(Icon)(({ theme, type }) => {
    const colorMap = {
...
export const SBadgeSlot = styled.div `
  ...
  height: 50%;
`;
export const SDecorationContainer = styled.div(({ decoration = "none", count, size }) => {
    ...
    `,
        ...
        decoration === "atoken" &&
            css `
        &::before {
          mask-image: ${generateATokenMask(count, diameter, overlap, thickness)};
        }
      `,
    ];
});
