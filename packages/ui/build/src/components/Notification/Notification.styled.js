...
export const SNotification = styled.div(({ theme }) => css `
    display: flex;
  ...
  `);
export const SProgressContainer = styled.div(({ theme }) => css `
    height: 2px;
    width: 100%;
    ...
    bottom: 0;
  `);
const getColor = (cssKey) => createVariants((theme) => ({
    success: css `
...
export const SProgress = styled.div(({ closeTime, variant }) => [
    css `
    @keyframes shrink {
      from {
        width: 100%;
...
export const SIconVariant = styled(Icon)(({ variant }) => [
    css `
      flex-shrink: 0;
    `,
    getColor("color")(variant),
]);
export const SCloseIcon = styled(Icon)(({ theme }) => css `
    position: absolute;
    top: 4px;
    right: 5px;
