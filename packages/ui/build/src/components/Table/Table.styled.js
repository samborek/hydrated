...
const pinnedColumnStyles = createVariants(() => ({
    left: css `
    left: 0;
  `,
    right: css `
...
const borderStyles = createStyles((theme) => css `
    tbody tr {
...
export const TableContainer = styled(Box) `
  ...
  position: relative;
`;
export const Table = styled.table(({ theme, size = "medium", fixedLayout = false, borderless = false }) => {
    return [
...
export const TableRow = styled.tr(({ theme, isClickable, isExpandable, hasOverride, isEmptyState }) => css `
    &[data-expanded="true"],
    ...
    `};
  `);
export const TableCell = styled.td(({ theme, isPinned, isClickable }) => {
    if (isPinned) {
        return [
...
export const TableHead = styled.th(({ theme, canSort, isPinned, isSorting }) => css `
    text-align: start;
    font-weight: 500;
