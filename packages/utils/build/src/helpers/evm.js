...
export const isH160Address = (address) => !!address && isAddress(address);
export function isEvmParachainAccount(address) {
    if (!address)
        return false;
    try {
        const pub = decodeAddress(address, true);
        return Buffer.from(pub.subarray(0, H160_PREFIX_BYTES.length)).equals(H160_PREFIX_BYTES);
    }
    catch {
        return false;
    }
}
export const safeConvertAddressH160 = (value) => {
    try {
...
export const safeConvertH160toSS58 = (address) => {
    if (!isH160Address(address))
        return "";
    try {
        const addressBytes = Buffer.from(address.slice(2), "hex");
        return encodeAddress(new Uint8Array(Buffer.concat([H160_PREFIX_BYTES, addressBytes, Buffer.alloc(8)])), 0);
    }
    catch {
        return "";
    }
};
export const safeConvertSS58toH160 = (address) => {
    try {
        if (isEvmParachainAccount(address)) {
            ...
            return (safeConvertAddressH160(Buffer.from(addressBytes).toString("hex")) ?? "");
        }
        else {
            const decodedBytes = decodeAddress(address);
...
export const getAssetIdFromAddress = (address) => {
    if (!isH160Address(address))
        return "";
    try {
...
export const getAddressFromAssetId = (assetId) => {
    try {
