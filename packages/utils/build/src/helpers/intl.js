...
const formatNumberParts = (part) => {
    if (part.type === "group") {
        return NB_SPACE;
    }
    return part.value;
};
const formatFractionDigits = (value, maximumFractionDigits, parts) => {
...
export const getMaxSignificantDigits = (value, options) => {
    ...
    return Math.min(21, (numberBig.gt(99999.9999) ? 0 : numberBig.gt(999.9999) ? 2 : 4) +
        intPartLen);
};
export const formatNumber = (value, lng = DEFAULT_LOCALE, options = {}) => {
    if (!isValidNumber(value)) {
        return NA_VALUE;
    }
    const numericValue = typeof value === "string" ? Number(value) : value;
    return new Intl.NumberFormat(lng, {
        maximumSignificantDigits: options.maximumSignificantDigits || options.maximumFractionDigits
            ? undefined
...
export const formatPercent = (value, lng = DEFAULT_LOCALE, options = {}) => {
    if (!isValidNumber(value)) {
        return NA_VALUE;
    }
    const percentage = Big(value.toString());
    const isBelowThreshold = percentage.gt(0) && percentage.lt(MIN_PERCENTAGE_THRESHOLD);
...
export const formatCurrency = (_value, lng = DEFAULT_LOCALE, options = {}) => {
    ...
    if (options.symbol) {
        parts = [
            ...
            { type: "currency", value: options.symbol },
        ];
    }
    if (options.maximumFractionDigits) {
        return formatFractionDigits(numericValue, Number(options.maximumFractionDigits), parts);
    }
    return parts.map(formatNumberParts).join("");
};
const FORMAT_ASSET_AMOUNT_REGEXP = /^(?<integer>[^.,]*)(?<separator>[.,])(?<decimals>.*)$/;
export const formatAssetAmount = (amount, maxDecimals) => {
