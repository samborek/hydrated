import { createZustandStorage, safeConvertSS58toPublicKey, } from "@galacticcouncil/utils";
...
export const useAddressStore = create()(persist((set) => ({
    ...defaultState,
    add: (address) => set((state) => {
        const addresses = Array.isArray(address) ? address : [address];
        const addressesSet = new Set(state.addresses.map((a) => a.publicKey));
        const uniqueAddresses = addresses.filter((a) => !addressesSet.has(a.publicKey));
    ...
    edit: (address) => set((state) => ({
        addresses: state.addresses.map((a) => a.publicKey === address.publicKey ? address : a),
    })),
    remove: (publicKey) => set((state) => ({
