...
export const ProviderExternalButton = () => {
    ...
    const { getStatus, accounts } = useWeb3Connect(useShallow(pick(["accounts", "getStatus"])));
    ...
    return (_jsx(ProviderButton, { ...walletData, onClick: () => {
            if (isConnected) {
                disconnect(provider);
            }
            else {
        ...
        }, isConnected: isConnected, accountCount: accountCount, actionLabel: isConnected ? "Disconnect" : "Continue" }));
};
