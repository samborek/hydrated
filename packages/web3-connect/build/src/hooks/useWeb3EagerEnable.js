...
import { isH160Address, isSS58Address, safeConvertSS58toH160, } from "@galacticcouncil/utils";
...
export const useWeb3EagerEnable = (enabled = true) => {
    ...
    const { providers, setAccount } = useWeb3Connect(useShallow(pick(["providers", "setAccount"])));
    ...
    useEffect(() => {
        ...
        async function eagerEnable() {
            if (hasTriedEagerEnable.current)
                return;
            for (const { type } of providers) {
                ...
                const isExternalConnected = account?.provider === WalletProviderType.ExternalWallet;
