...
export const useWeb3Enable = () => {
    const { setStatus, setError, disconnect, setAccounts, setAccount } = useWeb3Connect(useShallow(pick([
        "setStatus",
        "setError",
        "disconnect",
        "setAccounts",
    ...
    const unsubAndDisconnect = useCallback((type) => {
    ...
    const { mutateAsync: enable, ...mutation } = useMutation({
        ...
        onSuccess: (data, type) => {
            ...
            const addresses = data
                ...
                .filter(({ provider }) => provider !== WalletProviderType.ExternalWallet);
