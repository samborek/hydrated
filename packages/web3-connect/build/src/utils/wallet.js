import { EvmAddr, isEvmParachainAccount, isH160Address, safeConvertAddressSS58, safeConvertH160toSS58, safeConvertSolanaAddressToSS58, safeConvertSS58toH160, safeConvertSS58toPublicKey, safeConvertSuiAddressToSS58, SolanaAddr, Ss58Addr, } from "@galacticcouncil/utils";
import { allAccountFilterOptions, } from "@/components/account/AccountFilter";
import { SOLANA_PROVIDERS, SUI_PROVIDERS, WalletProviderType, } from "@/config/providers";
import { COMPATIBLE_WALLET_PROVIDERS, PROVIDERS_BY_WALLET_MODE, useWeb3Connect, WalletMode, } from "@/hooks/useWeb3Connect";
const toStoredSolanaAccount = ({ address, name, provider, }) => {
...
const toStoredSuiAccount = ({ address, name, provider, }) => {
...
const toStoredDefaultAccount = ({ address, name, provider, }) => {
...
export const toStoredAccount = ({ address, name, provider, }) => {
    switch (true) {
...
export const getAccountAvatarTheme = (account) => {
    if (account.provider === WalletProviderType.Talisman ||
...
export const getWalletModeByAddress = (address) => {
    switch (true) {
...
export const getDefaultAccountFilterByMode = (mode) => {
    if (mode !== WalletMode.Default)
        return (allAccountFilterOptions.find((option) => option === mode) ||
            WalletMode.Default);
    return WalletMode.Default;
};
export function subscribeWalletAccounts(wallet, { onDisconnect, onAccountsChange, onMainAccountChange, }) {
    const unsubscribe = wallet.subscribeAccounts((accounts) => {
        ...
        if (!currentAccount || currentAccount.provider !== wallet.provider)
            return;
        const isCurrentAccountConnected = accounts.some((a) => toStoredAccount(a).publicKey === currentAccount.publicKey);
...
export function getWalletModeIcon(mode) {
    switch (mode) {
        case WalletMode.EVM:
...
export function getWalletModesByProviderType(walletType) {
