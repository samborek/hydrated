import { shortenAccountAddress } from "@galacticcouncil/utils";
import { EthereumSigner } from "@/signers/EthereumSigner";
import { NotInstalledError } from "@/utils/errors";
export class BaseEIP1193Wallet {
    provider = "";
    accessor = "";
    title = "";
    installUrl = "";
    logo = "";
    _rawExtension;
    _extension;
    _signer;
    _enabled = false;
    constructor() {
        window.addEventListener("eip6963:announceProvider", (e) => this.handleAnnounceProvider(e));
    }
    get extension() {
        return this._extension;
    }
    get signer() {
        return this._signer;
    }
    get installed() {
        return !!this._rawExtension;
    }
    get enabled() {
        return this._enabled;
    }
    get rawExtension() {
        return this._rawExtension;
    }
    handleAnnounceProvider = (e) => {
        if (e.detail.info.rdns === this.accessor) {
            this._rawExtension = e.detail.provider;
        }
    };
    transformError = (err) => {
        return new Error(err.message);
    };
    enable = async () => {
        if (!this.installed || !this.rawExtension) {
            throw new NotInstalledError(`Refresh the browser if ${this.title} is already installed.`, this);
        }
        try {
            const extension = this.rawExtension;
            const addresses = await extension.request({
                method: "eth_requestAccounts",
            });
            const address = Array.isArray(addresses) && addresses.length > 0 ? addresses[0] : "";
            this._enabled = true;
            this._extension = extension;
            this._signer = address
                ? new EthereumSigner(address, extension)
                : undefined;
        }
        catch (err) {
            //@ts-expect-error unknown error type
            if (err.code === -32002) {
                throw new NotInstalledError(`Already processing request from ${this.title}. Check your wallet.`, this);
            }
            throw this.transformError(err);
        }
    };
    getAccounts = async () => {
        if (!this._extension) {
            throw new NotInstalledError(`Refresh the browser if ${this.title} is already installed.`, this);
        }
        const accounts = (await this._extension.request({
            method: "eth_requestAccounts",
        }));
        return (accounts || [])
            .filter((address) => !!address)
            .slice(0, 1)
            .map(this.toWalletAccount);
    };
    toWalletAccount = (address) => {
        return {
            address: address,
            provider: this.provider,
            name: shortenAccountAddress(address),
        };
    };
    subscribeAccounts = (callback) => {
        const extension = this._extension;
        if (!extension) {
            throw new NotInstalledError(`Refresh the browser if ${this.title} is already installed.`, this);
        }
        const handler = (payload) => {
            const addresses = Array.isArray(payload)
                ? payload.map((item) => item)
                : [];
            const accounts = addresses.slice(0, 1).map(this.toWalletAccount);
            callback?.(accounts);
        };
        extension.on("accountsChanged", handler);
        return () => {
            extension.removeListener("accountsChanged", handler);
        };
    };
    disconnect = () => {
        this._enabled = false;
        this._extension = undefined;
        this._signer = undefined;
    };
}
